 ...
 */
fetch(`${API_URL}/programs/${programId}/weeks`).then(res => res.json())
/**
...
.then(weeks => {
  ...
  weeks.forEach(week => {
    ...
    promises.push(fetch(`${API_URL}/weeks/${week.id}/sessions`).then(res => res.json()));
  });
...
.then(sessionsLists => {
  ...
  sessions.forEach(session => {
    ...
    promises.push(fetch(`${API_URL}/sessions/${session.id}/session-items`).then(res => res.json()));
  });
  return Promise.all(promises);
})
